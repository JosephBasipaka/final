...
class FinalRestControllerTest {
    ...
    void testCustomer() {
        ...
        ResponseEntity<List<Customer>> response = restTemplate.exchange(
                createURLWithPort("/customers"), HttpMethod.GET, entity,
    ...
    void testOrderById() throws JsonProcessingException {
        ...
        ResponseEntity<Customer> response = restTemplate.exchange(
                (createURLWithPort("/customer/id?name=john&email=jo@gmail.com")), HttpMethod.GET, entity,
                new ParameterizedTypeReference<Customer>() {
                });
        Customer customer = response.getBody();
        String outcome = "{\"id\":22,\"name\":\"john\",\"email\":\"jo@gmail.com\",\"invoices\":[],\"services\":[],\"paymentPlan\":[],\"payment\":[]}";
    ...
    void testOrderByIdFail() throws JsonProcessingException {
        ...
        ResponseEntity<Long> response = restTemplate.exchange(
                (createURLWithPort("/customer/id?name=jo&email=jo@gmail.com")), HttpMethod.GET, entity,
    ...
    void testCreateCustomer() throws JsonProcessingException {
        Customer customer = new Customer(40L, "j", "j@gmail.com", null, null, null, null);
    ...
    void testDunningIntitate() throws JsonProcessingException {
        UriComponentsBuilder builder = UriComponentsBuilder.fromPath("/dunning/intReminder")
                .queryParam("reminderType", "Final Notice");
        HttpEntity<String> entity = new HttpEntity<>(null, headers);
        ResponseEntity<String> response = restTemplate.exchange(
                createURLWithPort(builder.toUriString()), HttpMethod.POST, entity,
                new ParameterizedTypeReference<String>() {
                });
        dunningScheduler.configSchedulerManually("Final Notice");
        assertEquals(200, response.getStatusCode().value());
        assertEquals("Scheduled Final%20Notice reminder executed immediately.", response.getBody());
    }

    @Test
    @Sql(statements = { "INSERT INTO customer(id, name, email) VALUES (200, 'kim', 'kim@example.com')",
            "INSERT INTO service(id, service_cost, service_name, customer_id, status) VALUES (300, 1000, 'basic', 200, 'Active')" }, executionPhase = Sql.ExecutionPhase.BEFORE_TEST_METHOD)
    @Sql(statements = { "DELETE FROM service WHERE id='300'",
            "DELETE FROM customer WHERE id='200'" }, executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
    void testGetServiceId() throws JsonProcessingException {
        UriComponentsBuilder builder = UriComponentsBuilder.fromPath("/service/id")
                .queryParam("customerId", "200")
                .queryParam("serviceName", "basic");
        HttpEntity<String> entity = new HttpEntity<>(null, headers);
        ResponseEntity<String> response = restTemplate.exchange(
                createURLWithPort(builder.toUriString()), HttpMethod.GET, entity,
                new ParameterizedTypeReference<String>() {
                });
        serviceService.findId(200L, "basic");
        assertEquals(200, response.getStatusCode().value());
    }

    @Test
    void testGetServiceId_False() throws JsonProcessingException {
        UriComponentsBuilder builder = UriComponentsBuilder.fromPath("/service/id")
                .queryParam("customerId", "200")
                .queryParam("serviceName", "basic");
        HttpEntity<String> entity = new HttpEntity<>(null, headers);
    ...
    @Sql(statements = { "DELETE FROM service WHERE id='300'",
            "DELETE FROM customer WHERE id='200'" }, executionPhase = Sql.ExecutionPhase.AFTER_TEST_METHOD)
    void testGetCustomer() throws JsonProcessingException {
        UriComponentsBuilder builder = UriComponentsBuilder.fromPath("/find")
                .queryParam("customerName", "kim")
                .queryParam("serviceName", "basic");
        HttpEntity<String> entity = new HttpEntity<>(null, headers);
        ResponseEntity<Customer> response = restTemplate.exchange(
                createURLWithPort(builder.toUriString()), HttpMethod.GET, entity,
        ...
        customerService.getCustomerAndService("kim", "basic");
        assertEquals(200, response.getStatusCode().value());
    }

    @Test
    void testGetCustomerFails() throws JsonProcessingException {
        UriComponentsBuilder builder = UriComponentsBuilder.fromPath("/find")
                .queryParam("customerName", "kim")
                .queryParam("serviceName", "basic");
        HttpEntity<String> entity = new HttpEntity<>(null, headers);
